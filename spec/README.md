...
1. **Changes start here, not in the codebase (e.g. `cmd/`, `internal/`).** A fix, feature, or behavior change is first written as an edit to the relevant file under `spec/`. Code is a generated/derived artifact of the spec, not the other way around.
...
7. **Drift**: `spec/` and the codebase should never silently diverge. If code is changed directly (hotfix, exploratory patch), the corresponding spec file(s) must be updated in the same change before it's considered done.
