...
Sideload, list, and delete course files (`.fit`/`.gpx`) on a detected Garmin watch using standard filesystem operations, relying on the host OS to handle mounts (FUSE for Linux GVFS, macOS Volumes, or Windows standard mass storage drives).
...
- Given a filename, attempt standard filesystem unlink in `courses_dir` and `newfiles_dir`.
