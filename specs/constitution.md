...
| **FCT-6** | XML Standard | `GarminDevice.xml` utilizes XML namespaces. Go's `encoding/xml` matches elements by local name when struct tags omit the namespace, so no explicit namespace-stripping step is needed during unmarshaling. | XML file inspection; confirmed by successful `encoding/xml` unmarshal of a real, namespaced `GarminDevice.xml` (2026-09-17, PR #29) | Informs `FR-2`, `FR-5` |
...
| **FCT-13** | OS (Linux) | GVFS FUSE mounts for MTP devices do not support standard POSIX writes in practice — confirmed for both `open(O_CREAT)` on a new file and plain `O_WRONLY` on an already-existing file, both returning `EOPNOTSUPP`. GVFS D-Bus Push (e.g. via `gio copy`) is the documented remedy, but has **not yet been empirically confirmed to work end-to-end**: a 2026-09-17 run observed `gio copy` fail identically ("Operation not supported") for both new and existing destination files. It is unresolved whether that failure reflects a genuine limitation of the D-Bus-push path or an artifact of running inside a sandboxed background job whose D-Bus/session-bus isolation differs from the interactive desktop session that owns the GVFS mount. | Empirical verification on GVFS MTP mount; `EOPNOTSUPP` confirmed directly via Go and Python `open()`/`os.open()` and via `gio copy` CLI (PR #29, 2026-09-17) | Informs `FR-6`, `FR-9` |
...
- **FR-10: Web GUI Course Route & Elevation Preview**: Provide an offline vector route map and elevation profile preview for course files (`.gpx`, `.fit`) within the web GUI, computing distance, elevation gain/loss, and track geometry for both device courses and staged uploads.
